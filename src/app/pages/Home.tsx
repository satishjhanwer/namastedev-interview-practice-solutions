              ...
              >
                <CardActionArea to={`/s/${it.slug}`} component={RouterLink} sx={{ height: '100%', display: 'flex', alignItems: 'stretch' }}>
                  ...
                  >
                    ...
                    <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                      {it.meta?.difficulty && (
                        <Chip
                          ...
                          label={it.meta.difficulty}
                          onClick={(e) => {
                            e.preventDefault();
                            onChipClick(it.meta!.difficulty);
                          }}
                        />
                      )}
                      {it.meta?.status && (
                        <Chip
                          color={chipColor(it.meta.status)}
                          label={it.meta.status}
                          onClick={(e) => {
                            e.preventDefault();
                            onChipClick(it.meta!.status);
                          }}
                        />
                      )}
                    ...
                    <Typography color="text.secondary">{it.meta?.description ?? ''}</Typography>
